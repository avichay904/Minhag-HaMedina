...
# 2. Build images and start all services (postgres + api + web + admin)
docker compose up --build

# 3. Services are available at:
#    REST API   → http://localhost:3000/api/v1
#    Web SPA    → http://localhost:8080
#    Admin SPA  → http://localhost:8081
...
- The `web` and `admin` containers wait for the `api` healthcheck to pass
  before starting. Both build their SPAs with `VITE_API_BASE_URL` baked in at
  build time, then serve the result via nginx with SPA fallback.
...
3. Render creates a Postgres database and three services (`mhm-api`, `mhm-web`,
   `mhm-admin`).
...
| `VITE_API_BASE_URL` | web, admin (build arg) | YES | Full URL of the deployed API, e.g. `https://mhm-api.onrender.com`. Baked into the JS bundle at build time — both `mhm-web` and `mhm-admin` consume this. |
...
# Web SPA + Admin SPA — API URL seen by the browser (baked in at build time)
